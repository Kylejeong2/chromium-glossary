...
export function TerminalApp({ onExit }: { onExit: () => void }) {
  ...
  const [output, setOutput] = useState<Output[]>([{ id: 0, lines: ["Chromium desktop", "Type help to see available commands."] }]);
  ...
  return <div className="terminal-app"><div className="terminal-app__meta"><span>guest@browserbase</span><a href={CAREERS_URL} target="_blank" rel="noreferrer">Careers ↗</a></div><div className="terminal-app__output" role="log" aria-live="polite">{output.map((item) => <div key={item.id}>{item.prompt !== undefined && <p><b>guest@browserbase</b> % {item.prompt}</p>}{item.lines.map((line) => <p key={line}>{line}</p>)}{item.link && <p><a href={item.link.href} target="_blank" rel="noreferrer">{item.link.label}</a></p>}</div>)}</div><form onSubmit={submit}><label htmlFor="terminal-command">guest@browserbase %</label><input id="terminal-command" autoFocus autoComplete="off" value={input} onChange={(event) => setInput(event.target.value)} /></form></div>;
}
