...
const assets = [
  ...
  "public/assets/wallpapers/glass-ribbons-gold.jpg",
...
function isDiagramRooted(selector: string): boolean {
  if (!selector.startsWith(".concept-diagram")) return false;
  const boundary = selector[".concept-diagram".length];
  if (boundary && !/[.#:\[\s>+~]/.test(boundary)) return false;

  let parentheses = 0;
  let brackets = 0;
  let quote = "";
  for (let index = ".concept-diagram".length; index < selector.length; index += 1) {
...
describe("visual asset provenance", () => {
  it("ships every declared asset with revision-pinned provenance", () => {
    ...
    for (const record of records) {
      ...
      expect(record).toMatch(/^- (?:Source|Terminal source): (?:https:\/\/|Generated with OpenAI image generation)/m);
  ...
  it("scopes GT Standard Mono to concept diagrams and keeps Inter everywhere else", () => {
    ...
    expect(scopeViolations(css, /font-family:\s*"GT Standard Mono"/)).toEqual([]);
  });
  ...
  it("rejects duplicate font declarations and selectors that target outside the diagram root", () => {
    const css = readFileSync("src/app/globals.css", "utf8");
    expect(scopeViolations(`${css}\nbody { font-family: "GT Standard Mono"; }`, /font-family:\s*"GT Standard Mono"/)).toEqual(["body"]);
