...
describe("glossary routes", () => {
  it("parses desktop, index, and entry locations", () => {
    ...
    expect(parseGlossaryPath("/glossary", "?stage=process-boundaries")).toEqual({ open: true, stage: "process-boundaries" });
...
describe("Chrome navigation", () => {
  ...
  it("accepts only the glossary, known stages, and known local slugs", () => { const slugs = new Set(["site-isolation"]); const stages = new Set(["process-boundaries"]); expect(parseLocalAddress("chromium://glossary", "https://example.test", slugs, stages)).toBe("/glossary"); expect(parseLocalAddress("chromium://glossary?stage=process-boundaries", "https://example.test", slugs, stages)).toBe("/glossary?stage=process-boundaries"); expect(parseLocalAddress("chromium://glossary/site-isolation", "https://example.test", slugs, stages)).toBe("/glossary/site-isolation"); expect(parseLocalAddress("https://example.test/glossary/site-isolation", "https://example.test", slugs, stages)).toBe("/glossary/site-isolation"); expect(parseLocalAddress("https://google.com", "https://example.test", slugs, stages)).toBeUndefined(); expect(parseLocalAddress("chromium://glossary?stage=missing", "https://example.test", slugs, stages)).toBeUndefined(); expect(parseLocalAddress("chromium://glossary/missing", "https://example.test", slugs, stages)).toBeUndefined(); });
});
